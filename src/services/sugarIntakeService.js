...
class SugarIntakeService {
  ...
  async getIntakesListLastWeek(username) {
    try {
      ...
      const result = await SugarIntake.populate(intakeList, {
        path: "_id",
        model: "PackagedFood",
      });
  ...
  async foodPrediction_byTime(username) {
    try {
      ...
      const user_id = user._id;
      const foodList_intervals = await utils.getTopIntakesList_byTimeIntervals(
        user_id
      );
      let foodListCurrent = await utils.getTopIntakeFoodIds_CurrentInterval(
        foodList_intervals
      );
      ...
      if (foodListCurrent.length < 5) {
        ...
        if (response && response.foodlist) {
          ...
          const foodListSecStrings = foodListSec.map((foodIdObj) =>
            foodIdObj.toString()
          );
      ...
      if (foodListCurrent.length > 5) {
        foodListCurrent = foodListCurrent.slice(0, 5);
      }
      console.log(foodListCurrent);
      if (foodListCurrent.length > 0) {
        const foodAndServingPromise = foodListCurrent.map((food_id) =>
          utils.getMostFrequentServingCount(user_id, food_id)
        );
        const recommendFoodAndServingCount = await Promise.all(
          foodAndServingPromise
        );
        return { success: true, predictions: recommendFoodAndServingCount };
      } else {
        return {
          success: false,
          message: "No food data available for this time interval.",
        };
      }
    } catch (error) {
      console.error("Error in getting food prediction!", error);
